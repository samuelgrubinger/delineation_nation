...
blup_mod = function(df) {
  ...
  mm = lmer(var_value ~ (1|Prov_f:Site_f) + # random effect of each family at each site - the blups we're interested in
              (1|Blk_f:(Rep_f:Site_f)) + # random effect of block within rep within site
              (1|Rep_f:Site_f) # random effect of each rep at each site
            + Site_f, # site as a fixed effect
            data = df, REML = TRUE)
...
by_pop = pols_dat_dbh %>%
  ...
  mutate(Group_lab = case_when(Group == 'Field Traits' ~ 'Ground',
...
# PC1: temperature
(pc_plot = pca_clim$rotation %>% 
  as.data.frame() %>% 
  rownames_to_column() %>% 
    dplyr::select(rowname:PC3) %>% 
    pivot_longer(cols = PC1:PC3, names_to = "PC", values_to = "loading") %>% 
    mutate(rowname = dplyr::recode(rowname, MAT = 'MAT (mean annual temp.)',
    ...
    group_by(PC) %>% 
    mutate(rowname = reorder_within(rowname, abs(loading), PC)) %>% 
  ...
  theme(axis.text.x = element_text(family = "Cambria", size = 15),
        axis.text.y = element_text(family = "Cambria", hjust = 0, vjust = .5, size = 14),
    ...
    facet_wrap(. ~ PC, scales = "free_y", nrow = 3))
...
ggsave(plot = pc_plot,
       filename = 'D:\\Sync\\Figures\\_FIGURES_GCB\\PC_loadings.jpeg',
       device = jpeg,
       width = 11,
       height = 15,
       units = 'in',
...
(pc1_plot = pca_clim$rotation %>% 
    ...
    geom_bar(stat = "identity") +
    labs(x = "",
         y = "Loadings for PC1 (cold–warm)") +
    theme_bw(base_size = 22) +
    scale_x_discrete(labels = clim_labs) +
    coord_flip() +
    theme(axis.text.x = element_text(family = "Cambria", size = 20),
          axis.text.y = element_text(family = "Cambria", hjust = 0, vjust = .5, size = 23),
          axis.title.x = element_text(family = "Cambria", size = 26, vjust = 1,
...
ggsave(plot = pc2_plot,
       filename = 'D:\\Sync\\Figures\\_FIGURES_GCB\\PC2.jpeg',
       device = jpeg,
       width = 15,
       height = 15,
       units = 'in',
...
(pc3_plot = pca_clim$rotation %>% 
  ...
  geom_bar(stat = "identity") +
    labs(x = "",
         y = "Loadings for PC3 (lowland–montane)") +
    theme_bw(base_size = 22) +
    scale_x_discrete(labels = clim_labs) +
    coord_flip() +
    theme(axis.text.x = element_text(family = "Cambria", size = 20),
...
ggsave(plot = pc3_plot,
       filename = 'D:\\Sync\\Figures\\_FIGURES_GCB\\PC3.jpeg',
       device = jpeg,
       width = 15,
       height = 15,
       units = 'in',
...
saveRDS(all_plot, "D:\\Sync\\_Sites\\Sx_Genecology\\_GCB_share\\PCA_sites_seedlots_R.rds")
write.csv(all_plot, "D:\\Sync\\_Sites\\Sx_Genecology\\_GCB_share\\PCA_sites_seedlots.csv")
...
(by_pop = pols_dat_dbh %>%
  ...
  left_join(sites_pc, by = c("Site" = "Site")) %>% 
  left_join(pops_pc, by = c("Prov" = "Prov")) %>% 
  mutate(`PC1 (warm–cold)` = PC1_site - PC1,
         `PC2 (arid–moist)` = PC2_site - PC2,
         `PC3 (lowland–montane)` = PC3_site - PC3) %>% 
  pivot_longer(cols = `PC1 (warm–cold)`:`PC3 (lowland–montane)`,
               names_to = "PC", values_to = "distance"))
...
by_site_corr = by_pop %>%
  ...
  mutate(cline = case_when(PC == "PC1 (warm–cold)" & sign == "neg" ~ "warm",
                           PC == "PC1 (warm–cold)" & sign == "pos" ~ "cold",
                           PC == "PC2 (arid–moist)" & sign == "neg" ~ "arid",
                           PC == "PC2 (arid–moist)" & sign == "pos" ~ "moist",
                           PC == "PC3 (lowland–montane)" & sign == "neg" ~ "lowland",
                           PC == "PC3 (lowland–montane)" & sign == "pos" ~ "arid"),
...
by_site_corr_save = by_site_corr %>% 
  filter(mean_type == "BLUP") %>% 
  dplyr::select(-zone, -species, -Weev16, -Rep, -Blk, -site_mean_weev)

saveRDS(by_site_corr_save, "D:\\Sync\\_Sites\\Sx_Genecology\\_GCB_share\\blups_correlations_R.rds")
write.csv(by_site_corr_save, "D:\\Sync\\_Sites\\Sx_Genecology\\_GCB_share\\blups_correlations.csv")
...
by_site_best_plot = by_site_corr %>%
  filter(variable == 'mean.CCI' & mean_type == 'BLUP')
...
by_site_best_sig = filter(by_site_best_plot, p_plot != "NS") %>% 
  ...
  mutate(lab_dist = case_when(sign == "neg" & min(distance) > -5 ~ max(distance) - 5.5,
                              sign == "neg" & min(distance) <= -5 ~ min(distance) + 1,
...
(g5 = by_site_best_plot %>% 
    ...
    labs(x = 'PC transfer distance',
         y = 'Chlorophyll Carotenoid Index (CCI) BLUPs') +
         # y = expression('Near-infrared' ~ reflectance ~ of ~ vegetation ~
         #                  (NIR[V ^ NDRE]) ~ BLUPs)) +
    #      # expression(bold(Slope~of~the~upper~red~edge~(RE[UPPER]))))+
    scale_y_continuous(expand = expansion(add = c(.008,.02)), breaks = seq(-.04, .04, .02)) +
    ...
    theme(legend.position = 'none',  
          ...
          plot.background = element_rect(color = 'white', fill = NA, linewidth = 1),  
          ...
          strip.text = element_text(size = 17, face = "bold", family = "Cambria")) +
    facet_grid(PC ~ Site,
               #ncol = 3, 
               scales = "free"))
...
by_site_best_map = all_plot %>% 
  filter(type == "Seedlot") %>% 
  dplyr::select(name, geometry) %>% 
  left_join(by_site_corr %>%
              filter(variable == 'mean.CCI' & mean_type == 'BLUP'), by = c("name" = "Prov")) %>%
...
(trait_map = ggplot() +
    ...
    geom_sf(data = states, fill = NA) +
    geom_sf(data = ocean, fill = "white", color = "grey20") + 
    geom_sf(data = by_site_best_map, aes(color = var_value), shape = 16, size = 3.5) +
    scale_color_viridis_c(option = "C", labels = scales::number_format(accuracy = 0.5)) +
    #geom_sf(data = sites, color = "black", size = 3.5, shape = 21) +
    coord_sf(xlim = c(225000, 2800000), ylim = c(-768333, 2150000),
             crs = st_crs(3005)) +
...
(g6 = by_site_corr_sum %>% 
    ...
    mutate(Group_lab = ordered(Group_lab, 
                               levels = c("Ground", "Structural", "Spectral"))) %>% 
    ...
    theme(legend.position = 'none',
          ...
          axis.title.x = element_text(size = 22, margin = margin(t = 12, r = 0, b = 0, l = 0),
                                      family = 'Cambria'),
          strip.text = element_text(size = 15, face = "bold", family = "Cambria"),
...
ggsave(plot = g6,
       filename = 'D:\\Sync\\Figures\\_FIGURES_GCB\\figure_6_P1_prez.jpeg',
       device = jpeg,
       width = 15,
       height = 5,
       units = 'in',
...
by_site_corr_rank = by_site_corr_sum %>% 
  filter(mean_type == 'BLUP') %>% 
  mutate(pc_prop = case_when(PC == "PC1 (warm–cold)" ~ prop_var[1],
                             PC == "PC2 (arid–moist)" ~ prop_var[2],
                             PC == "PC3 (lowland–montane)" ~ prop_var[3])) %>% 
...
val_mods_save = filter(val_mods, model_name %in% c('model_6', 'mod_z'))

saveRDS(val_mods_save, "D:\\Sync\\_Sites\\Sx_Genecology\\_GCB_share\\model_cross_validation_R.rds")
write.csv(val_mods_save, "D:\\Sync\\_Sites\\Sx_Genecology\\_GCB_share\\model_cross_validation.csv")
...
(g7 = ggplot(val_mods_sum_plot) +
  geom_abline(slope = 1, intercept = 0, color = 'black') +  
    geom_point(data = arrange(val_mods_plot, desc(Prov)), aes(y = .fitted, x = DBH16, color = Class), color = "steelblue4", size = 3.4, alpha = .5 
               #color = '#607080'
    ) +  
...
(s1 = ggplot(val_mods_sum_plot) +
    geom_abline(slope = 1, intercept = 0, color = 'black') +  
    geom_point(data = arrange(val_mods_plot, desc(Prov)), aes(y = .fitted, x = HT16, color = Class), color = "steelblue4", size = 3.4, alpha = .5 
               #color = '#607080'
                 ) +  
    ...
    theme_bw(base_size = 18) +
    scale_color_manual(values = class_cols) +
    labs(x = 'Ground-measured height BLUPs (m)',  
...
saveRDS(halfnorm_plot, "D:\\Sync\\_Sites\\Sx_Genecology\\_GCB_share\\halfnormal_R.rds")
write.csv(dplyr::select(halfnorm_plot, -model), "D:\\Sync\\_Sites\\Sx_Genecology\\_GCB_share\\halfnormal.csv")

halfnorm_table = halfnorm_plot %>% 
  # mutate(x = max(Euc),
  #        y = max(BLUP)) %>% 
  dplyr::select(Site, Class, model_name, AIC, sigma, par_a, par_b) %>% 
  distinct() %>% 
...
plot_labs = c("orch_meas" = "",
              "orch_pred" = "",
              "wild_meas" = "Ground-measured",
              "wild_pred" = "Model-predicted")

legend_title = "  Orchard     Wildstand"


(g8 = halfnorm_plot %>% filter(Site == 'Kalamalka'#'Tête Jaune Cache'
                                 ) %>%
...
ggsave(plot = g8,
       filename = 'D:\\Sync\\Figures\\_FIGURES_GCB\\figure_8_Kalamalka.tiff',
       device = tiff,
       width = 15,
       height = 10,
       units = 'in',
...
ggsave(plot = g8,
       filename = 'D:\\Sync\\Figures\\_FIGURES_GCB\\figure_8_Kalamalka.jpeg',
       device = jpeg,
       width = 15,
       height = 10,
       units = 'in',
...
(g8 = halfnorm_plot %>% filter(Site == 'Tête Jaune Cache') %>%
  ...
  scale_size_manual(name = legend_title,
                    values = c('orch_meas' = 2.5,
                               ...
                               'wild_pred' = 4.4),
                    labels = plot_labs) +
  theme(legend.position = c(0.835, 0.93),
        legend.background = element_rect(fill = "white", color = "black", size = .5),
        panel.border = element_rect(fill = NA, color = "black", size = 1),
        legend.key.width = unit(2.75,"cm"),legend.key.height = unit(.7,"cm"),
        legend.margin = margin(4, 4, 4, 4),
        legend.spacing.x = unit(.1, 'cm'),
        strip.text = element_text(size = 20, family = "Cambria"),
        strip.placement = "left",
        strip.background.y = element_blank(),
        strip.switch.pad.wrap = unit(.3, 'cm'),
        legend.title = element_text(size = 17, family = "Cambria"),
  ...
  #scale_y_continuous(expand = expansion(mult = c(.06, .16))) + #Kalamalka
  #scale_y_continuous(expand = expansion(mult = c(.06, .13))) + #TJC
  scale_y_continuous(expand = expansion(mult = c(.08, .18))) + #Skim, Whit
  facet_wrap(Class ~ phen, scales = 'free', strip.position = "left"))

ggsave(plot = g8,
       filename = 'D:\\Sync\\Figures\\_FIGURES_GCB\\figure_8_TJC_pub.tiff',
       device = tiff,
       width = 15,
       height = 10,
       units = 'in',
...
ggsave(plot = g8,
       filename = 'D:\\Sync\\Figures\\_FIGURES_GCB\\figure_8_TJC_pub.jpeg',
       device = jpeg,
       width = 15,
       height = 10,
       units = 'in',
...
#############
# ground_AIC = halfnorm_plot %>%
#   filter(legend_id %in% c("orch_meas", "wild_meas")) %>% 
#   mutate(AIC_ground = AIC) %>% 
#   group_by(Site, Class, phen, AIC_ground)%>% 
#   summarise()

halfnorm_sigma = halfnorm_plot %>%
  dplyr::select(Site, Class, phen, sigma, legend_id) %>% 
  mutate(sigma = round(sigma, 2)) %>% 
  distinct() %>% 
  mutate(sig_lab = case_when(legend_id == "orch_pred" ~ "RSE[model]",
                         legend_id == "orch_meas" ~ "RSE[ground]",
                         legend_id == "wild_pred" ~ "RSE[model]",
                         legend_id == "wild_meas" ~ "RSE[ground]"),
         x_pos = if_else(Site == "Harrison", .6, 5.5),
         y_pos = if_else(Site %in% c("Harrison", "Whitecourt"), 40, 83)) %>% 
  mutate(y_pos = case_when(legend_id == "orch_pred" ~ y_pos + 11,
                                 legend_id == "orch_meas" ~ y_pos + 5,
                                 legend_id == "wild_pred" ~ y_pos - 5,
                                 legend_id == "wild_meas" ~ y_pos - 11))
...
(g8 = halfnorm_plot %>% 
   ...
   filter(phen == "DBH (mm)") %>% 
    group_by(Site, phen) %>% 
    mutate(y_pos = max(pred) * 1.1,
           x_pos = min(Euc) + 3,
           vju = if_else(Class == "Orchard", -2.5, 0)) %>% 
   ggplot(aes(x = Euc, y = TRAIT, color = legend_id, alpha = legend_id, shape = legend_id, linetype = legend_id, group = legend_id)) +
   #geom_point(aes(size = legend_id)) +
   labs(x = 'Climate transfer distance (Euclidean units)',
        y = 'DBH (mm)') +
   #geom_smooth(size = 1.3, alpha = 1, se = FALSE, method = 'lm') +
   geom_line(aes(y = pred), linewidth = 1.1, show.legend = FALSE) +
    geom_rect(data = filter(halfnorm_sigma, phen == "DBH" & legend_id == 'orch_meas'),
              aes(x = 1, y = 40, 
                  xmin = x_pos - .18, xmax = x_pos + 3.8,
                 ymin = y_pos - 20, ymax = y_pos + 10),
              color = "black", fill = "white", show.legend = FALSE) +
    geom_text(data = filter(halfnorm_sigma, phen == "DBH"),
      aes(label = paste0(sig_lab, " == ", sigma), x = x_pos + .75, y = y_pos),
      size = 4.5,
      parse = TRUE,
      hjust = 0,
      color = "black",
   ...
   scale_size_manual(name = legend_title,
                     values = c('orch_meas' = 2.5,
                                ...
                                'wild_pred' = 4.4),
                     labels = plot_labs) +
   theme(legend.position = c(0.899, 0.395),
         legend.background = element_rect(fill = "white", color = "black", size = .5),
         panel.border = element_rect(fill = NA, color = "black", size = 1),
         legend.key.width = unit(1.5, "cm"),legend.key.height = unit(.7,"cm"),
         legend.margin = margin(4, 4, 4, 4),
         legend.spacing.x = unit(.1, 'cm'),
         strip.text = element_text(size = 20, family = "Cambria", face = "bold"),
         strip.placement = "left",
         strip.background = element_rect(fill = "white"),
         strip.switch.pad.wrap = unit(.3, 'cm'),
         legend.title = element_text(size = 10.5, family = "Cambria"),
         legend.text = element_text(size = 10.5, family = "Cambria"),
         axis.text = element_text(family='Cambria', size = 18, color = 'black'),  
         axis.title.y = element_text(vjust = 1.5, size = 24, family = "Cambria"),
         axis.title.x = element_text(vjust = -.5, size = 24, family = "Cambria")) +
   guides(color = guide_legend(ncol = 2)) +
    scale_x_continuous(breaks = seq(0, 8, 2)) +
   scale_y_continuous(expand = expansion(mult = c(0.04, .05)),
                      breaks = seq(25, 100, 25)) + #Skim, Whit
   facet_wrap(. ~ Site, strip.position = "top", nrow = 2))

ggsave(plot = g8,
       filename = 'D:\\Sync\\Figures\\_FIGURES_GCB\\figure_8_pub.tiff',
       device = tiff,
       width = 15,
       height = 10,
       units = 'in',
...
ggsave(plot = g8,
       filename = 'D:\\Sync\\Figures\\_FIGURES_GCB\\figure_8_pub.jpeg',
       device = jpeg,
       width = 15,
       height = 10,
       units = 'in',
...
halfnorm_sigma = halfnorm_plot %>%
  ...
  mutate(sig_lab = case_when(legend_id == "orch_pred" ~ "RSE[model]",
                             ...
                             legend_id == "wild_pred" ~ "RSE[model]",
                             legend_id == "wild_meas" ~ "RSE[ground]"),
         x_pos = if_else(Site == "Harrison", .6, 6),
         y_pos = if_else(Site %in% c("Harrison", "Whitecourt"), 2.8, 5.2)) %>% 
  mutate(y_pos = case_when(legend_id == "orch_pred" ~ y_pos + .45,
                           legend_id == "orch_meas" ~ y_pos + .2,
                           legend_id == "wild_pred" ~ y_pos - .2,
                           legend_id == "wild_meas" ~ y_pos - .45))
...
(g8 = halfnorm_plot %>% 
    ...
    #geom_smooth(size = 1.3, alpha = 1, se = FALSE, method = 'lm') +
    geom_line(aes(y = pred), linewidth = 1.1, show.legend = FALSE) +
    geom_rect(data = filter(halfnorm_sigma, phen == "Height" & legend_id == 'orch_meas'),
              aes(x = 3, y = 3,
                  xmin = x_pos - .18, xmax = x_pos + 3.8,
                  ymin = y_pos - .85, ymax = y_pos + .45),
              color = "black", fill = "white", show.legend = FALSE) +
    geom_text(data = filter(halfnorm_sigma, phen == "Height"),
              aes(label = paste0(sig_lab, " == ", sigma), x = x_pos + .75, y = y_pos),
              size = 4.5,
              parse = TRUE,
              hjust = 0,
    ...
    guides(color = guide_legend(ncol = 2)) +
    scale_x_continuous(breaks = seq(0, 8, 2)) +
    scale_y_continuous(expand = expansion(mult = c(0.05, .05))) + #Skim, Whit
    facet_wrap(. ~ Site, strip.position = "top", nrow = 2))

ggsave(plot = g8,
       filename = 'D:\\Sync\\Figures\\_FIGURES_GCB\\figure_8_pub_height.tiff',
       device = tiff,
       width = 15,
       height = 10,
       units = 'in',
...
ggsave(plot = g8,
       filename = 'D:\\Sync\\Figures\\_FIGURES_GCB\\figure_8_pub_height.jpeg',
       device = jpeg,
       width = 15,
       height = 10,
       units = 'in',
...
(table_halfnorm_pub = halfnorm_plot %>%
    ungroup() %>% 
    dplyr::select(Site:Class, phen, legend_id, sigma, par_a, par_b) %>% 
    distinct() %>% 
  arrange(desc(phen)) %>% 
  mutate(across(sigma:par_b, round, 2)) %>% 
    distinct() %>%
    pivot_longer(cols = c('sigma', 'par_a', 'par_b'), names_to = 'fit_metric', values_to = 'fit_value') %>%  
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
save.image("model_pca_data.Rdata")
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
...
# p_1 = ggplot() +
#   geom_point(data = dat1, size = .2, aes(x = Y, y = Z), color = "grey20", alpha = .2) +
#   geom_point(data = dat2, size = .2, aes(x = Y, y = Z), color = "grey20", alpha = .2) +
#   geom_point(data = dat3, size = .2, aes(x = Y, y = Z), color = "grey20", alpha = .2) +
#   # geom_point(data = dat1, size = .4, aes(x = Y, y = Z, color = RGB)) +
#   # geom_point(data = dat2, size = .4, aes(x = Y, y = Z, color = RGB)) +
#   # geom_point(data = dat3, size = .4, aes(x = Y, y = Z, color = RGB)) +
#   geom_polygon(data = mesh1, fill = "grey25", alpha = .2,
#                aes(x = y_new, y = z, group = zorder)) +
#   geom_polygon(data = mesh2, fill = "grey25", alpha = .2,
#                aes(x = y_new, y = z, group = zorder, alpha = alpha_)) +
#   geom_polygon(data = mesh3, fill = "grey25", alpha = .2,
#                aes(x = y_new, y = z, group = zorder, alpha = alpha_)) +
#   scale_alpha(range = c(.1, .4)) +
#   geom_line(data = mesh1, size = .5, aes(x = y_new, y = z, group = zorder, alpha = alpha_)) +
#   geom_line(data = mesh2,  size = .5, aes(x = y_new, y = z, group = zorder, alpha = alpha_)) +
#   geom_line(data = mesh3,  size = .5, aes(x = y_new, y = z, group = zorder, alpha = alpha_)) +
#   # annotate(geom = "text",
#   #          color = "black",
...
p_4 = ggRGB(brick(tree_rgb), r = 1, g = 2, b = 3,
            ...
            stretch = "lin",
            quantiles = c(0, 1)) +
  #geom_sf(data = filter(trees_pols, Plot == "Corrected"), color = "red3", fill = NA, size = .7) +
  #labs(title = "Drone technology") +
  # scale_color_manual(values = c("Watershed" = "orange", "Corrected" = "red"),
  #                    labels = c("Watershed" = "Watershed  ", "Corrected" = "Corrected")) +
  theme_void() +
  scale_x_continuous(expand = c(0, 0)) +
  scale_y_continuous(expand = c(0, 0)) +
  # annotate(geom = "label", label = "COMMON-GARDEN TRIALS", x = ext[1] + 1, y = ext[4] - 1,
  #          hjust = 0, vjust = 1, family = "Cambria", fontface = "bold", color = "grey15",
  #          label.padding = unit(0.5, "lines"),
  #          label.r = unit(0, "lines")) +
  theme(panel.border = element_rect(color = "black", fill = NA, size = .8),
        legend.position = c(.18, .97),
        legend.justification = c(0, 1),
        legend.title.align = 1,
        # plot.title = element_text(size = 25, margin = margin(t = 0, r = 0, b = 20, l = 0),
        #                           hjust = .5, family = 'Cambria'),
...
p_6 = ggR(raster(cci_tree_stretch),
  ...
  scale_x_continuous(expand = c(0, 0)) + 
  scale_y_continuous(expand = c(0, 0)) +
  #labs(title = "Spectral traits") +
  theme(panel.border = element_rect(color = "black", fill = NA, size = .8),
        legend.position = "none",#c(.1, .97),
        legend.justification = c(0, 1),
        legend.title.align = 1,
        # plot.title = element_text(size = 25, margin = margin(t = 0, r = 0, b = 20, l = 0), 
        #                           hjust = .5, family = 'Cambria'),
...
tree_chm_smooth = rast(paste0(dir, "\\output\\RASTER\\", name, "_CHM_max_smooth.tif")) %>% 
  crop(ext)
quants = global(tree_chm_smooth, fun = quantile, probs = c(0, .97), na.rm = TRUE)
tree_chm_stretch = clamp(tree_chm, upper = quants[[2]], lower = quants[[1]])

(p_7 = ggR(raster(tree_chm),
    geom_raster = TRUE) +
  scale_fill_continuous(#limits = c(0, 6),
                        na.value = "black",
                        high = "white",
                        low = "black",                   
                        #breaks = seq(1, 7, 2),
...
(g1 = ggarrange(NULL, p_4, NULL, p_7, NULL, p_6, NULL,
                nrow = 1, ncol = 7,
                widths = c(.02, 1, .02, 1, .02, 1, .02)))

# g2 = ggarrange(NULL, p_4, NULL, g1,
#                nrow = 2, ncol = 5,
#                heights = c(.01, 1, .01, 1, .01))
# g3 = ggarrange(NULL, g1, NULL, g5, NULL, nrow = 5, ncol = 1, 
#                heights = c(.03, .75, .1, 1, .001))

ggsave(plot = g1, 
       filename = "D:\\Sync\\Figures\\_FIGURES_GCB\\graphical_abstract.jpeg", 
       device = jpeg,
       width = 15,
       height = 5,
       units = "in",
...
# by_site_best_plot = by_site_corr %>%
#   filter(variable == 'mean.CCI' & mean_type == 'BLUP' & Site == "Harrison" & PC == "PC1 (warm–cold)")
# 
# summary_stats = by_site_best_plot %>% 
#   group_by(Site, PC) %>% 
#   mutate(mean_d = mean(distance),
...
#    theme_bw(base_size = 15) +
#    labs(x = 'PC transfer distance',
#         y = 'Chlorophyll carotenoid index (CCI)') +
#    # y = expression('Near-infrared' ~ reflectance ~ of ~ vegetation ~
#    #                  (NIR[V ^ NDRE]) ~ BLUPs)) +
#    #      # expression(bold(Slope~of~the~upper~red~edge~(RE[UPPER]))))+
#   #expand_limits(x = 1) +
...
#              hjust = 0,
#              family = "Cambria") +
#     #labs(title = "Climate transfer clines") +
#    # geom_text(data = by_site_best_sig,
#    #           aes(x = lab_dist + 4.3),
#    #           y = Inf,
#    #           label = by_site_best_sig$p_plot,
...
#    theme(legend.position = 'none',  
#          strip.background = element_rect(fill = 'white'),  
#          axis.text = element_text(family='Cambria', size=12, color = 'black'),  
#          plot.background = element_rect(color = 'white', fill = NA, linewidth = 1),  
#          # plot.title = element_text(size = 28, margin = margin(t = 0, r = 0, b = 20, l = 0), 
#          #                           hjust = .5, family = 'Cambria'),
#          axis.text.y = element_text(margin = margin(r = 4, l = -4)),  
#          plot.margin = margin(.4, 1.7, .4, .4, 'cm'),  
#          panel.border = element_rect(color = 'black', fill = NA, linewidth = 1),  
#          axis.title.y = element_text(size = 18, margin = margin(t = 0, r = 12, b = 0, l = -10), family = 'Cambria', face = "bold"),  
#          axis.title.x = element_text(size = 18, margin = margin(t = 12, r = 0, b = 0, l = 0), family = 'Cambria', face = "bold"),  
#          strip.text = element_text(size = 20, face = "bold", family = "Cambria"),
...
#val_mods_plot2 = val_mods_plot
#val_mods_sum_plot2 = val_mods_sum_plot

# val_mods_plot = filter(val_mods_plot2, Site == "Jordan River")
# val_mods_sum_plot = filter(val_mods_sum_plot2, Site == "Jordan River")
# 
# (g7 = ggplot(val_mods_sum_plot) +
#     geom_abline(slope = 1, intercept = 0, color = 'black') +  
#     geom_point(data = arrange(val_mods_plot, desc(Prov)), aes(y = .fitted, x = DBH16, color = Class), color = "steelblue4", size = 3.4, alpha = .5 
